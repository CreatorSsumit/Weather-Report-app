...
function home(props) {
    ...
    var convdatawhole = month+'-'+day+'-'+year+' '+hours + ':' + minutes.substr(-2) + ':' + seconds.substr(-2);
   ...
   <View style={{paddingTop:10}}>
    ...
    <Text style={{fontSize:15,textAlign:"center",color:"white",padding:10}}>{convdatawhole}</Text>
        
          <View style={{width:"70%",marginLeft:20,marginTop:2,borderRadius:10,padding:20}}>
          <Text style={{fontSize:80,marginLeft:10,marginTop:10,color:"white"}}>
            <Text style={{color:"#435561",fontSize:100}}>2</Text>1.C
          </Text>
          <Text style={{fontSize:20,marginLeft:10, color:"#435561"}}>{currentdata.weather[0].description} ⛈⛈</Text>
          <TouchableOpacity  style={{marginTop:30}}><Text>Clik to update</Text></TouchableOpacity> 
          </View>
        </View>
        <View  style={{width:"94.5%",height:"45%",position:"absolute",bottom:"11%",left:"3%",borderRadius:20, backgroundColor:"#435561",opacity:0.85,paddingBottom:10,paddingTop:20}}>
    ...
    <Text style={{padding:5}}>{currentdata.main.humidity}</Text>
          </View>
    ...
    <Text style={{padding:5}}>{currentdata.main.pressure}</Text>
          </View>
        </View>
        
        <View style={{padding:13,borderBottomWidth:2,borderBottomColor:"#435561"}}></View>
        
        <Scrolldetail list = {climatedata.list} />
